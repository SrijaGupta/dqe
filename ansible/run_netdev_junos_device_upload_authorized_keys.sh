...
ANSIBLE_PLAYBOOK=netdev_junos_device_upload_authorized_keys.yml
...
DEVICE_GROUP_NAME=cluster_fabric_devices


if [ "$NETDEV_DEVICE_IP_LIST" == "" ] ; then
    echo "ERROR: NETDEV_DEVICE_IP_LIST env-var must be set - comma seprated device ip list-"
    exit 1
fi
...
${DEVICE_NAME}  ansible_host=${DEVICE_IP} ansible_user=${NETDEV_DEVICE_CLI_USERNAME}  x_ansible_ssh_private_key_file=${NETDEV_DEVICE_CLI_USER_AUTH_PRIV_KEY_FILE_PATH}  ansible_password=${NETDEV_DEVICE_CLI_PASSWORD}
EOF
done
cat >> $NETDEV_DEVICE_INVENTORY_FILE_PATH << EOF
[${DEVICE_GROUP_NAME}]
